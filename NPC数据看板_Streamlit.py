                ...
                if idx < len(row):
                    ...
                    if metric_name == "曝光量&CPC":
                        ...
                        def get_weekly_cpc(year):
                            ...
                            return filtered_df.groupby("周次").apply(lambda x: x["花费"].sum() / x["点击量"].sum() if x["点击量"].sum() != 0 else 0).rename("CPC").reset_index()
                        ...
                        yoy_df['last_na'] = yoy_df["曝光量_last"].isna()
                        yoy_df['current_na'] = yoy_df["曝光量_current"].isna()
                        valid_mask = (~yoy_df['last_na']) & (~yoy_df['current_na']) & (yoy_df["曝光量_current"] != 0) & (yoy_df["曝光量_last"] != 0)
                        yoy_df = yoy_df[valid_mask].copy()
                        if not yoy_df.empty:
                            yoy_df["yoy"] = ((yoy_df["曝光量_current"] - yoy_df["曝光量_last"]) / yoy_df["曝光量_last"]) * 100
                            yoy_df["mom_current"] = yoy_df["曝光量_current"].pct_change() * 100
                        else:
                            yoy_df["mom_current"] = pd.Series(dtype=float)
                            yoy_df["yoy"] = pd.Series(dtype=float)
                        ...
                        if yoy_df.empty:
                            with col:
                                st.info(f"{metric_name}：无有效对比周次")
                            continue
                        
                        text_current = [f"{int(v):,}" if pd.notna(v) else "" for v in yoy_df["曝光量_current"]]
                        text_last = [f"{int(v):,}" if pd.notna(v) else "" for v in yoy_df["曝光量_last"]]
                        
                        fig = make_subplots(specs=[[{"secondary_y": True}]])
                        fig.add_trace(go.Bar(x=yoy_df["周次"], y=yoy_df["曝光量_last"], name=f"{last_year}年", marker_color=html_colors[2], text=text_last, textposition="outside"), secondary_y=False)
                        fig.add_trace(go.Bar(x=yoy_df["周次"], y=yoy_df["曝光量_current"], name=f"{selected_year}年", marker_color=html_colors[0], text=text_current, textposition="outside"), secondary_y=False)
                        fig.add_trace(go.Scatter(x=yoy_df["周次"], y=yoy_df["CPC_current"], mode='lines+markers', name="CPC",
                                                 line=dict(color=html_colors[1]), hovertemplate='%{y:.2f} ¥<extra></extra>'), secondary_y=False)
                        fig.add_trace(go.Scatter(x=yoy_df["周次"], y=yoy_df["CPC_last"], mode='lines+markers', name="CPC (去年)",
                        ...
                        yoy_df['last_na'] = yoy_df["点击率_last"].isna()
                        yoy_df['current_na'] = yoy_df["点击率_current"].isna()
                        valid_mask = (~yoy_df['last_na']) & (~yoy_df['current_na']) & (yoy_df["点击率_current"] != 0) & (yoy_df["点击率_last"] != 0)
                        yoy_df = yoy_df[valid_mask].copy()
                        if not yoy_df.empty:
                            yoy_df["yoy"] = ((yoy_df["点击率_current"] - yoy_df["点击率_last"]) / yoy_df["点击率_last"]) * 100
                            yoy_df["mom_current"] = yoy_df["点击率_current"].pct_change() * 100
                        else:
                            yoy_df["mom_current"] = pd.Series(dtype=float)
                            yoy_df["yoy"] = pd.Series(dtype=float)
                        ...
                        fig = make_subplots(specs=[[{"secondary_y": True}]])
                        fig.add_trace(go.Scatter(x=yoy_df["周次"], y=yoy_df["点击率_last"], mode='lines+markers', name=f"{last_year}年 CTR",
                                                 line=dict(color=html_colors[2]), hovertemplate='%{y:.1f}%<extra></extra>'), secondary_y=False)
                        fig.add_trace(go.Scatter(x=yoy_df["周次"], y=yoy_df["点击率_current"], mode='lines+markers', name=f"{selected_year}年 CTR",
                                                 line=dict(color=html_colors[0]), hovertemplate='%{y:.1f}%<extra></extra>'), secondary_y=False)
                        fig.add_trace(go.Scatter(x=yoy_df["周次"], y=yoy_df["CR_last"], mode='lines+markers', name=f"{last_year}年 CR",
                                                 line=dict(color=html_colors[3]), hovertemplate='%{y:.1f}%<extra></extra>'), secondary_y=False)
                        fig.add_trace(go.Scatter(x=yoy_df["周次"], y=yoy_df["CR_current"], mode='lines+markers', name=f"{selected_year}年 CR",
                        ...
                        yoy_df['last_na'] = yoy_df[f"{agg_col}_last"].isna()
                        yoy_df['current_na'] = yoy_df[f"{agg_col}_current"].isna()
                        valid_mask = (~yoy_df['last_na']) & (~yoy_df['current_na']) & (yoy_df[f"{agg_col}_current"] != 0) & (yoy_df[f"{agg_col}_last"] != 0)
                        yoy_df = yoy_df[valid_mask].copy()
                        if not yoy_df.empty:
                            yoy_df["yoy"] = ((yoy_df[f"{agg_col}_current"] - yoy_df[f"{agg_col}_last"]) / yoy_df[f"{agg_col}_last"]) * 100
                        ...
                        if metric_name in ["销售额", "客单价", "广告花费", "广告销售额"]:
                            fig.add_trace(go.Bar(x=yoy_df["周次"], y=yoy_df[f"{agg_col}_last"], name=f"{last_year}年", marker_color=html_colors[2], text=text_last, textposition="outside"), secondary_y=False)
                            fig.add_trace(go.Bar(x=yoy_df["周次"], y=yoy_df[f"{agg_col}_current"], name=f"{selected_year}年", marker_color=html_colors[0], text=text_current, textposition="outside"), secondary_y=False)
                        else:
                            fig.add_trace(go.Scatter(x=yoy_df["周次"], y=yoy_df[f"{agg_col}_last"], mode='lines+markers', name=f"{last_year}年", line=dict(color=html_colors[2]), hovertemplate=hovertemplate), secondary_y=False)
                            fig.add_trace(go.Scatter(x=yoy_df["周次"], y=yoy_df[f"{agg_col}_current"], mode='lines+markers', name=f"{selected_year}年", line=dict(color=html_colors[0]), hovertemplate=hovertemplate), secondary_y=False)
