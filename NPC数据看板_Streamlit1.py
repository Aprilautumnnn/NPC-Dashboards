...
<style>
    ...
    .metric-label {
        width: 140px; padding: 12px; font-weight: bold; color: #9fd8ff;
        border-right: 1px solid rgba(255, 255, 255, 0.10);
    }
    .metric-value {
        flex: 1; padding: 12px; text-align: center; color: #FFFFFF;
    }
    .metric-header {
        background: rgba(255, 255, 255, 0.05); font-weight: bold; color: #FFFFFF;
    }
    .kpi-card {
        background: rgba(255, 255, 255, 0.07);
        border: 1px solid rgba(255, 255, 255, 0.10);
    ...
    .kpi-value { font-size: 22px; font-weight: bold; color: #FFFFFF; margin: 5px 0; }
    .kpi-label { font-size: 13px; color: #9fd8ff; }
...
def load_data():
    try:
        ...
        # 数值列
        numeric_cols = ["销售额/RMB", "销量", "毛利润", "经营利润", "花费", 
                       "曝光量", "点击量", "订单量", "广告单量", "广告销售额"]
        for col in numeric_cols:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)
        
        # 派生指标（用于每行数据）
        df["客单价"] = df["销售额/RMB"] / df["销量"].replace(0, 1)
        df["毛利率"] = (df["毛利润"] / df["销售额/RMB"]).replace([np.inf, -np.inf], 0) * 100
        df["经营利润率"] = (df["经营利润"] / df["销售额/RMB"]).replace([np.inf, -np.inf], 0) * 100
        df["净利润"] = df["经营利润"] - df["花费"]  # 净利润 = 经营利润 - 花费
        df["净利率"] = (df["净利润"] / df["销售额/RMB"]).replace([np.inf, -np.inf], 0) * 100
        df["退款补发率"] = ((df["毛利润"] - df["经营利润"]) / df["销售额/RMB"]).replace([np.inf, -np.inf], 0) * 100
        df["ACOAS"] = (df["花费"] / df["销售额/RMB"]).replace([np.inf, -np.inf], 0) * 100
        df["ACOS"] = (df["花费"] / df["广告销售额"]).replace([np.inf, -np.inf], 0) * 100
        df["ASOAS"] = (df["广告销售额"] / df["销售额/RMB"]).replace([np.inf, -np.inf], 0) * 100
...
if not df.empty:
    ...
    available_categories = sorted(df["类目名称"].unique())
    ...
    def calc_metrics(df_group):
        total_sales = df_group["销售额/RMB"].sum()
        ...
        total_ad_sales = df_group["广告销售额"].sum()
        
        aur = total_sales / total_quantity if total_quantity > 0 else 0
    ...
    # 其他指标无同比，仅展示本周值；近六周和YTD为累计/平均值
    # 注意：经营利润率、净利率、退款补发率、ACOAS 在近六周和YTD中使用加权平均（实际与累计计算一致，因为 calc_metrics 已按总额计算比率）
    ...
    rows = [
        ("GMV", 
         f"¥{week_metrics['GMV']:,.0f}", 
         f"¥{last6_metrics['GMV']:,.0f}", 
         f"¥{ytd_metrics['GMV']:,.0f}"),
        ("GMV YoY", 
         f"{week_metrics['GMV_YoY']:.1f}%", 
         f"{last6_metrics['GMV_YoY']:.1f}%", 
         f"{ytd_metrics['GMV_YoY']:.1f}%"),
        ("毛利润", 
         f"¥{week_metrics['毛利润']:,.0f}", 
         f"¥{last6_metrics['毛利润']:,.0f}", 
         f"¥{ytd_metrics['毛利润']:,.0f}"),
        ("毛利润 YoY", 
         f"{week_metrics['毛利润_YoY']:.1f}%", 
         f"{last6_metrics['毛利润_YoY']:.1f}%", 
         f"{ytd_metrics['毛利润_YoY']:.1f}%"),
        ("毛利率", 
         f"{week_metrics['毛利率']:.1f}%", 
         f"{last6_metrics['毛利率']:.1f}%", 
         f"{ytd_metrics['毛利率']:.1f}%"),
        ("毛利率 YoY", 
         f"{week_metrics['毛利率_YoY']:.1f}%", 
         f"{last6_metrics['毛利率_YoY']:.1f}%", 
         f"{ytd_metrics['毛利率_YoY']:.1f}%"),
        ("经营利润率", 
         f"{week_metrics['经营利润率']:.1f}%", 
         f"{last6_metrics['经营利润率']:.1f}%", 
         f"{ytd_metrics['经营利润率']:.1f}%"),
        ("净利率", 
         f"{week_metrics['净利率']:.1f}%", 
         f"{last6_metrics['净利率']:.1f}%", 
         f"{ytd_metrics['净利率']:.1f}%"),
        ("退款补发率", 
         f"{week_metrics['退款补发率']:.1f}%", 
         f"{last6_metrics['退款补发率']:.1f}%", 
         f"{ytd_metrics['退款补发率']:.1f}%"),
        ("ACOAS", 
         f"{week_metrics['ACOAS']:.1f}%", 
         f"{last6_metrics['ACOAS']:.1f}%", 
         f"{ytd_metrics['ACOAS']:.1f}%"),
    ]
    ...
    <div class="metric-table">
        <div class="metric-row metric-header">
            ...
            <div class="metric-value">近六周</div>
            <div class="metric-value">YTD</div>
        </div>
    """
    for label, w, s6, y in rows:
        table_html += f"""
        <div class="metric-row">
            <div class="metric-label">{label}</div>
            <div class="metric-value">{w}</div>
            <div class="metric-value">{s6}</div>
    ...
    with col1:
        market_sales = filtered_df.groupby("销售市场")["销售额/RMB"].sum().reset_index()
        fig_country = px.pie(market_sales, values="销售额/RMB", names="销售市场", 
    ...
    with col2:
        platform_sales = filtered_df.groupby("平台")["销售额/RMB"].sum().reset_index()
        platform_sales = platform_sales[platform_sales["销售额/RMB"] > 0]
        fig_platform = px.pie(platform_sales, values="销售额/RMB", names="平台",
    ...
    with col1:
        status_sales = filtered_df.groupby("销售状态")["销售额/RMB"].sum().reset_index()
        status_sales = status_sales.sort_values("销售额/RMB", ascending=True)
        fig_status = px.bar(status_sales, x="销售额/RMB", y="销售状态", orientation='h',
                            title="各销售状态销售额",
                            labels={"销售额/RMB": "销售额 (¥)", "销售状态": ""},
    ...
    with col2:
        if "系列名称" in filtered_df.columns:
            series_sales = filtered_df.groupby("系列名称")["销售额/RMB"].sum().reset_index()
        else:
            series_sales = filtered_df.groupby("类目名称")["销售额/RMB"].sum().reset_index()
            series_sales.rename(columns={"类目名称": "系列名称"}, inplace=True)
        
        series_sales = series_sales.sort_values("销售额/RMB", ascending=False).head(10)
        series_sales = series_sales.sort_values("销售额/RMB", ascending=True)
        fig_series = px.bar(series_sales, x="销售额/RMB", y="系列名称", orientation='h',
                            title="系列销售额 TOP10",
                            labels={"销售额/RMB": "销售额 (¥)", "系列名称": ""},
    ...
    ad_trend_df["ACOAS"] = (ad_trend_df["花费"] / ad_trend_df["销售额/RMB"] * 100).fillna(0)
    ad_trend_df["ASOAS"] = (ad_trend_df["广告销售额"] / ad_trend_df["销售额/RMB"] * 100).fillna(0)
    ...
    profit_trend_df["毛利率"] = (profit_trend_df["毛利润"] / profit_trend_df["销售额/RMB"] * 100).fillna(0)
    profit_trend_df["经营利润率"] = (profit_trend_df["经营利润"] / profit_trend_df["销售额/RMB"] * 100).fillna(0)
    ...
    summary_table = filtered_df.groupby(["销售市场", "平台", "类目名称", "销售状态"]).agg({
        "销售额/RMB": ["sum", "mean"],
        "销量": "sum",
        "毛利润": "sum",
        "经营利润": "sum",
