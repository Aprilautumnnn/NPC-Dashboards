...
if not df.empty:
    ...
    # 填充 Week 列
    with col1:
        st.markdown('<div class="metric-card" style="background: rgba(255,255,255,0.12); font-weight: bold;">Week</div>', unsafe_allow_html=True)
        for metric in metric_names:
            st.markdown(f"""
            <div class="metric-card">
                <div class="metric-label">{metric}</div>
                <div class="metric-value">{week_data[metric]}</div>
            </div>
            """, unsafe_allow_html=True)
    
    # 填充 近六周 列
    with col2:
        st.markdown('<div class="metric-card" style="background: rgba(255,255,255,0.12); font-weight: bold;">近六周</div>', unsafe_allow_html=True)
        for metric in metric_names:
            st.markdown(f"""
            <div class="metric-card">
                <div class="metric-label">{metric}</div>
                <div class="metric-value">{last6_data[metric]}</div>
            </div>
            """, unsafe_allow_html=True)
    
    # 填充 YTD 列
    with col3:
        st.markdown('<div class="metric-card" style="background: rgba(255,255,255,0.12); font-weight: bold;">YTD</div>', unsafe_allow_html=True)
        for metric in metric_names:
            st.markdown(f"""
            <div class="metric-card">
                <div class="metric-label">{metric}</div>
                <div class="metric-value">{ytd_data[metric]}</div>
            </div>
            """, unsafe_allow_html=True)
